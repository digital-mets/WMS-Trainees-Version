...
<body style="height: 910px">
                                    ...
                                    <dx:LayoutGroup Caption="General" ColCount="2">
                                        <Items>
                                            ...
                                            <dx:LayoutItem Caption="Reference Checks:" Name ="RC">
                                                <LayoutItemNestedControlCollection>
                                                    <dx:LayoutItemNestedControlContainer runat="server">
                                                        <dx:ASPxGridLookup ID="glReferenceChecks" runat="server" AutoGenerateColumns="False" DataSourceID="sdsCheckDetail" KeyFieldName="CheckNumber" OnLoad="LookupLoad" SelectionMode="Multiple" TextFormatString="{3}" Width="170px"  OnInit="glReferenceChecks_Init">
                                                            <GridViewProperties>
                                        ...
                                        <LayoutItemNestedControlCollection>
                                            <dx:LayoutItemNestedControlContainer runat="server">
                                                <dx:ASPxGridView ID="gv1" runat="server" AutoGenerateColumns="False" Width="850px" OnCellEditorInitialize="gv2_CellEditorInitialize"
                                                     ClientInstanceName="gv1"  OnBatchUpdate="gv1_BatchUpdate" KeyFieldName="DocNumber;LineNumber" OnCustomButtonInitialize="gv1_CustomButtonInitialize">
    ...
    <asp:SqlDataSource ID="sdsDetail" runat="server" ConnectionString="<%$ ConnectionStrings:GEARS-METSITConnectionString %>" SelectCommand="SELECT * FROM  Accounting.DepositSlipCash where DocNumber is null " OnInit="ConnectionInit_Init"></asp:SqlDataSource>
    ...
    <asp:SqlDataSource ID="sdsCheckDetail" runat="server" ConnectionString="<%$ ConnectionStrings:GEARS-METSITConnectionString %>" 
        SelectCommand="SELECT RIGHT('00000'+ CAST(ROW_NUMBER() OVER (ORDER BY CheckDate) AS VARCHAR(5)),5) AS LineNumber, *, '' AS Field1, '' AS Field2, '' AS Field3, '' AS Field4, '' AS Field5, '' AS Field6, '' AS Field7, '' AS Field8, '' AS Field9
    ...
    <%--SELECT RIGHT('00000'+ CAST(ROW_NUMBER() OVER (ORDER BY CheckDate) AS VARCHAR(5)),5) AS LineNumber, *, '' AS Field1, '' AS Field2, '' AS Field3, '' AS Field4, '' AS Field5, '' AS Field6, '' AS Field7, '' AS Field8, '' AS Field9
        FROM (SELECT A.DocNumber, A.DocNumber AS RefDocNumber, Bank AS BankCode, CheckNumber, Branch AS BankBranch, CAST(CheckDate AS DATE) AS CheckDate, CheckAmount FROM Accounting.CollectionChecks A INNER JOIN Accounting.Collection B ON A.DocNumber = B.DocNumber WHERE ISNULL(IsForDeposit,0) = 1 AND A.DepositedDate IS NULL AND ISNULL(CancelledCheckDocNum,'') = '' UNION ALL
	    SELECT A.DocNumber, A.DocNumber AS RefDocNumber, Bank AS BankCode, CheckNumber, Branch AS BankBranch, CAST(CheckDate AS DATE) AS CheckDate, CheckAmount FROM Accounting.CheckVoucherDetail A WHERE A.DocNumber IN (SELECT DocNumber FROM Accounting.CheckVoucher WHERE TransType = 'Fund Transfer') AND ClearedDate IS NULL AND ReleasedDate IS NULL AND ISNULL(CancelledCheckDocNum,'') = '') A--%>
    <asp:SqlDataSource ID="sdsCashDetail" runat="server" ConnectionString="<%$ ConnectionStrings:GEARS-METSITConnectionString %>" SelectCommand="SELECT DocNumber, DocNumber AS RefDocNum, ISNULL(CashAmount,0) AS Amount, B.Name, DocDate, ISNULL(CustomerCode,'') AS CustomerCode, A.Field1, A.Field2, A.Field3, A.Field4, A.Field5, A.Field6, A.Field7, A.Field8, A.Field9 FROM Accounting.Collection A LEFT JOIN Masterfile.BPCustomerInfo B ON A.CustomerCode = B.BizPartnerCode WHERE ISNULL(IsForDeposit,0) = 1 AND DepositedDate IS NULL AND CashAmount != 0" OnInit="ConnectionInit_Init"></asp:SqlDataSource>
