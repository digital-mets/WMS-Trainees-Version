...
function OnInitTrans(s, e) {
    ...
    //if (s == gvStepProcess && postback) {
    //    if (gvStepProcess.GetVisibleRowsOnPage() > 0)
    //        cbacker.PerformCallback('update|' + gvStepProcess.batchEditApi.GetCellValue(0, 'ItemCode'));
    //    postback = false;
    //}
}
    ...
    if (x == 0) {
        ...
        for (var i = 0; i < indicies.length; i++) {
            if (gvStepProcess.batchEditHelper.IsNewItem(indicies[i])) {
                ...
                if (gvStepProcess.batchEditHelper.IsDeletedItem(key))
                    var aa;
                else {
                    gvStepProcess.batchEditApi.ValidateRow(indicies[i]);
                    gvStepProcess.batchEditApi.StartEdit(indicies[i], gvStepProcess.GetColumnByField("SKUCode").index);
...
var editorobj;
var gvProdindex;
function gvStepProcess_OnStartEditing(s, e) {
    console.log(e);
    index = e.visibleIndex;
    currentColumn = e.focusedColumn;
    var cellInfo = e.rowValues[e.focusedColumn.index];
...
function gvStepProcess_OnEndEditing(s, e) {
    var cellInfo = e.rowValues[currentColumn.index];
    ...
    //    gvStepProcess.batchEditApi.StartEdit(gverrindex, gvStepProcess.GetColumnByField("ItemCode").index);
...
function GridEnd(s, e) {
    ...
    if (val != null && val != 'undefined' && val != '') {
        for (var i = 0; i < gvStepProcess.GetColumnsCount() ; i++) {
            ...
            if (column.visible == false || column.fieldName == undefined)
                continue;
            ProcessCells(0, editorobj, column, gvStepProcess);
...
function gvStepProcess_KeyDown(s, e) { //Allows tabbing between gridlookup on details
    ...
    if (gvStepProcess.batchEditApi[moveActionName]()) {
...
function gvStepProcess_KeyPress(s, e) { //Prevents grid refresh when a user press enter key for every column
    var keyCode = ASPxClientUtils.GetKeyCode(e.htmlEvent);
    if (keyCode == 13) {
        gvStepProcess.batchEditApi.EndEdit();
    }
...
function gvStepProcess_CloseUp(s, e) {
    gvStepProcess.batchEditApi.EndEdit();
}
...
function OnCustomClick(s, e) {
    ...
    if (e.buttonID == "StepProcessEdit") {
        gvStepProcess.StartEditRow(e.visibleIndex);
        //s.PerformCallback("asd|");
    } 

    if (e.buttonID == "ProductDetails") {
        var itemcode = gvStepProcess.batchEditApi.GetCellValue(e.visibleIndex, "ItemCode");
        var colorcode = gvStepProcess.batchEditApi.GetCellValue(e.visibleIndex, "ColorCode");
        var classcode = gvStepProcess.batchEditApi.GetCellValue(e.visibleIndex, "ClassCode");
        var sizecode = gvStepProcess.batchEditApi.GetCellValue(e.visibleIndex, "SizeCode");
        var fulldesc = gvStepProcess.batchEditApi.GetCellValue(e.visibleIndex, "FullDesc");
    ...
    if (e.buttonID == "ProductDelete") {
        gvStepProcess.DeleteRow(e.visibleIndex);
        if (gvStepProcess.GetVisibleRowsOnPage() == 0) {
    ...
    if (e.buttonID == "AddBillofMaterials") {
        var id = e.buttonID.split("Add")[1] + e.visibleIndex;
        var isExisting = $('#' + id).length;

        isExisting > 0 ? $('#' + id).modal('show') : createModal(id, "Bill of Materials SP" + e.visibleIndex);
    }

    if (e.buttonID == "AddMachineries") {
        var id = e.buttonID.split("Add")[1] + e.visibleIndex;
        var isExisting = $('#' + id).length;

        isExisting > 0 ? $('#' + id).modal('show') : createModal(id, "Machineries SP" + e.visibleIndex);
    }

    if (e.buttonID == "AddManpower") {
        var id = e.buttonID.split("Add")[1] + e.visibleIndex;
        var isExisting = $('#' + id).length;

        isExisting > 0 ? $('#' + id).modal('show') : createModal(id, "Manpower SP" + e.visibleIndex);
    }
...
function calculate_header(s, e) {
    ...
    setTimeout(function () {
        ...
        var iProduct = gvStepProcess.batchEditHelper.GetDataItemVisibleIndices();
        ...
        for (var i = 0; i < iProduct.length; i++) {
            if (gvStepProcess.batchEditHelper.IsNewItem(iProduct[i])) {

                DSOQty = gvStepProcess.batchEditApi.GetCellValue(iProduct[i], "SOQty");
                DJOQty = gvStepProcess.batchEditApi.GetCellValue(iProduct[i], "JOQty");

                HSOQty += DSOQty;
                HJOQty += DJOQty;
            }
            else {
                var key = gvStepProcess.GetRowKey(iProduct[i]);
                if (gvStepProcess.batchEditHelper.IsDeletedItem(key)) {
...
function calculate_BOM(s, e) {
    ...
    setTimeout(function () {
        ...
        var iProd = gvStepProcess.batchEditHelper.GetDataItemVisibleIndices();
            ...
            if (gvBOM.batchEditHelper.IsNewItem(key)) {
                ...
                if ((PrCode == null || PrCode == "") && (PrColor == null || PrColor == "") && (PrSize != null || PrSize != "")) {
                    ...
                    for (var y = 0; y < iProd.length; y++) {
                        ...
                        OrCode = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "ItemCode");
                        OrQty = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "JOQty");
                    ...
                    for (var y = 0; y < iProd.length; y++) {
                        ...
                        OrColor = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "ColorCode");
                        OrQty = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "JOQty");
                    ...
                    for (var y = 0; y < iProd.length; y++) {
                        ...
                        OrCode = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "ItemCode");
                        OrSize = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "SizeCode");
                        OrQty = gvStepProcess.batchEditApi.GetCellValue(iProd[y], "JOQty");
...
function getSize(s, e) {
    ...
    var igv1 = gvStepProcess.batchEditHelper.GetDataItemVisibleIndices();

    for (var g = 0; g < igv1.length; g++) {
        if (gvStepProcess.batchEditHelper.IsNewItem(igv1[g])) {
            if (sizeArr.length == 0) {
                sizeArr.push([gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode"), gvStepProcess.batchEditApi.GetCellValue(igv1[g], "JOQty")]);
            }
            else {
                if (isInArray(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode"), sizeArr)) {
                    sizeArr[findIndexArr(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode"), sizeArr)][1] += gvStepProcess.batchEditApi.GetCellValue(igv1[g], "JOQty");
                }
                else {
                    sizeArr.push([gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode"), gvStepProcess.batchEditApi.GetCellValue(igv1[g], "JOQty")]);
                }
            ...
            var key = gvStepProcess.GetRowKey(igv1[g]);
            if (gvStepProcess.batchEditHelper.IsDeletedItem(key)) {
...
function addSize(array) {
    ...
    for (var g = 0; g < igv1.length; g++) {
        if (gvStepProcess.batchEditHelper.IsNewItem(igv1[g])) {
            if (!isInArray2(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode"), array)) {
                gvSize.AddNewRow();
                items.push(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode"));
                items.push(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SOQty"))
                items.push(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "JOQty"))
                items.push(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "INQty"))
                items.push(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "FinalQty"))
...
function checkItem(s, e) {
    if (gvStepProcess.GetVisibleRowsOnPage() == 1 && PItem != gl.GetValue())
...
function gvProductRow(s, e) {
    ...
    if (gvStepProcess.batchEditApi.GetCellValue(gvProdindex, "ReferenceSO") == null || gvStepProcess.batchEditApi.GetCellValue(gvProdindex, "ReferenceSO") == '')
        for (var i = 0; i < gvStepProcess.GetColumnsCount() ; i++) {
            var column = s.GetColumn(i);
...
function validateRow() {
    ...
    for (var g = 0; g < igv1.length; g++) {
        if (igv1[g] != gvProdindex)
            if (gvStepProcess.batchEditHelper.IsNewItem(igv1[g])) {
                if (gvStepProcess.batchEditApi.GetCellValue(igv1[g], "ReferenceSO") != null)
                    skuarr.push(gvStepProcess.batchEditApi.GetCellValue(igv1[g], "ItemCode").trim() + ';'
                           + gvStepProcess.batchEditApi.GetCellValue(igv1[g], "ColorCode").trim() + ';' + gvStepProcess.batchEditApi.GetCellValue(igv1[g], "ClassCode").trim() + ';'
                           + gvStepProcess.batchEditApi.GetCellValue(igv1[g], "SizeCode").trim() + ';')
            }
            else {
                var key = gvStepProcess.GetRowKey(igv1[g]);
                if (gvStepProcess.batchEditHelper.IsDeletedItem(key)) {
    ...
    try {
        sku = gvStepProcess.batchEditApi.GetCellValue(gvProdindex, "ItemCode").trim() + ';'
                           + gvStepProcess.batchEditApi.GetCellValue(gvProdindex, "ColorCode").trim() + ';' + gvStepProcess.batchEditApi.GetCellValue(gvProdindex, "ClassCode").trim() + ';'
                           + gvStepProcess.batchEditApi.GetCellValue(gvProdindex, "SizeCode").trim() + ';';
    }
    catch (exception) {
        //console.log('err')
    }
...
function ValidateSteps() {
    ...
    for (var g = 0; g < igvStepPlan.length; g++) {
        if (igvStepPlan[g] != gvStepPlanIndex) {
            var key = gvStepProcess.GetRowKey(igvStepPlan[g]);
            if (!gvStepPlanning.batchEditHelper.IsDeletedItem(key)) {
...
function ShowSizeHorizontal(s, e) {
    ...
    var hIndex = gvStepProcess.batchEditHelper.GetDataItemVisibleIndices();
    ...
    for (var a = 0; a < hLength; a++) {
        ...
        item = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "ItemCode");
        color = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "ColorCode");
        cls = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "ClassCode");
        qty = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "JOQty");
        price = 0;
...
function CalculateSize(s, e) {
    ...
    setTimeout(function () {
        ...
        var key = gvStepProcess.GetRowKey(indicies[w]);
        if (!gvStepProcess.batchEditHelper.IsDeletedItem(key)) {
            for (var b = 1; b < col; b++) {
...
function PopulateSizes(s, e) {
    ...
    for (var x = 1; x < sCol; x++) {
        ...
        for (var a = 0; a < hLength; a++) {
            ...
            color = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "ColorCode");
            cls = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "ClassCode");
            size = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "SizeCode");
            qty = gvStepProcess.batchEditApi.GetCellValue(hIndex[a], "JOQty");
...
function CallbackSize(s, e) {
    ...
    var query = "";
    var cnt = 0;
    var gvIndex = gvStepProcess.batchEditHelper.GetDataItemVisibleIndices();
        ...
        if (qtyx != 0 && qtyx != "0.00" && qtyx != null && qtyx != "" && qtyx != '') {
            ...
            if (isInArray(checkitem, arrayGrid)) {
                ...
                for (var q = 0; q < gvLength; q++) {
                    var exist = "";
                    gItem = gvStepProcess.batchEditApi.GetCellValue(gvIndex[q], "ItemCode");
                    gColor = gvStepProcess.batchEditApi.GetCellValue(gvIndex[q], "ColorCode");
                    gClass = gvStepProcess.batchEditApi.GetCellValue(gvIndex[q], "ClassCode");
                    gSize = gvStepProcess.batchEditApi.GetCellValue(gvIndex[q], "SizeCode");
                    exist = gItem + '|' + gColor + '|' + gClass + '|' + gSize;

                    if (exist == checkitem) {
                        gvStepProcess.batchEditApi.SetCellValue(gvIndex[q], "JOQty", qtyx);
                    }
                }
            }
            else {
                gvStepProcess.AddNewRow();
                getColSH(gvStepProcess, editorobj, item);
            }
        }
...
function getColSH(ss, ee, item) {
    for (var i = 0; i < gvStepProcess.GetColumnsCount() ; i++) {
        ...
        if (column.visible == false || column.fieldName == undefined)
            continue;
        Bindgrid(item, ee, column, gvStepProcess);
    }
}
...
function checkGrid() {
    ...
    for (var w = 0; w < indicies.length; w++) {
        if (gvStepProcess.batchEditHelper.IsNewItem(indicies[w])) {
            Keyfield = gvStepProcess.batchEditApi.GetCellValue(indicies[w], "ItemCode") + '|' + gvStepProcess.batchEditApi.GetCellValue(indicies[w], "ColorCode") + '|' + gvStepProcess.batchEditApi.GetCellValue(indicies[w], "ClassCode") + '|' + gvStepProcess.batchEditApi.GetCellValue(indicies[w], "SizeCode");
            arrayGrid.push(Keyfield)
            gvStepProcess.batchEditApi.ValidateRow(indicies[w]);
        }
        else {
            var key = gvStepProcess.GetRowKey(indicies[w]);
            if (gvStepProcess.batchEditHelper.IsDeletedItem(key))
...
function SetStepCode(s, e) {
    var gridClientObject = s.GetGridView();
    gridClientObject.GetRowValues(gridClientObject.GetFocusedRowIndex(), "StepCode;Description", OnCallback);
}

function OnCallback(values) {
    var editorToUpdate2 = gvStepProcess.GetEditor("StepDescription");
    var editorToUpdate = gvStepProcess.GetEditor(3);
    editorToUpdate2.SetText(values[1]);
    editorToUpdate.SetText(values[0]);
    console.log(values);
