...
function OnCustomClick(s, e) {
    if (undefined == e.buttonID) {
        gvProductOrder.batchEditApi.SetCellValue(e.visibleIndex, "BOMCount", 0);
        gvProductOrder.batchEditApi.SetCellValue(e.visibleIndex, "MachCount", 0);
        gvProductOrder.batchEditApi.SetCellValue(e.visibleIndex, "ManCount", 0);
    }
...
function createRow(id, tableClass) {
    ...
    if (tableClass.search("Bill") > -1) {
        columnCount = 12;
        columnTitle = ["", "Item Code", "Item Description", "Step Code", "Unit", "Consumption Per Product", "Total Consumption", "% Allowance", "Quantity Allowance", "Client Supplied Material", "Estimated Unit Cost", "Standard Usage", "Remarks"];
    }

    if (tableClass.search("Machine") > -1) {
        columnCount = 7;
        columnTitle = ["", "Machine Types", "Machime Description", "Location", "Machine Run", "Unit", "Machine Capacity in Unit", "Cost Per Unit"];
    }

    if (tableClass.search("Man") > -1) {
        columnCount = 6;
        columnTitle = ["", "Designation", "No. of Manpowers", "No. of Hours", "Standard Rate", "Standard Rate Unit", "Cost Per Unit"];
    }
...
function updateCount(tableid) {
    if (tableid.search("Bill") > -1) gvProductOrder.batchEditApi.SetCellValue('-' + tableid.split("Table")[1], "BOMCount", $('#' + tableid + " .tbody tr").length);
    if (tableid.search("Machine") > -1) gvProductOrder.batchEditApi.SetCellValue('-' + tableid.split("Table")[1], "MachCount", $('#' + tableid + " .tbody tr").length);
    if (tableid.search("Man") > -1) gvProductOrder.batchEditApi.SetCellValue('-' + tableid.split("Table")[1], "ManCount", $('#' + tableid + " .tbody tr").length);
}
// Tables END