...
namespace Entity
{
    public class PettyCashFundSetUp
    {
        ...
        public DataTable getdata(string DocNumber, string Conn) //Ter
        {
            DataTable a;

            if (DocNumber != null)
            {
                a = Gears.RetriveData2("select * from Masterfile.PettyCashFundSetUp where FundCode = '" + DocNumber + "'", Conn); //Ter
        ...
        public void InsertData(PettyCashFundSetUp T)
        {
            ...
            Gears.CreateData(DT1, T.Connection); // TER

            Functions.AuditTrail("PETTYCSHFND", T.FundCode, T.AddedBy, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "INSERT", T.Connection); // TER
        }

        public void UpdateData(PettyCashFundSetUp L)
        {
            Conn = L.Connection; //Ter
            ...
            string strErr = Gears.UpdateData(DT1, L.Connection); // Ter

            Functions.AuditTrail("PETTYCSHFND", L.FundCode, LastEditedBy, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "UPDATE", L.Connection); // Ter
        }
        public void DeleteData(PettyCashFundSetUp V)
        {
            Conn = V.Connection; //Ter
            FundCode = V.FundCode;

            Gears.CRUDdatatable DT1 = new Gears.CRUDdatatable();
            DT1.Rows.Add("Masterfile.PettyCashFundSetUp", "cond", "FundCode", V.FundCode);
            Gears.DeleteData(DT1, V.Connection); // Ter
            Functions.AuditTrail("PETTYCSHFND", V.FundCode, LastEditedBy, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "DELETE", V.Connection); // Ter
        }
    }
}
